...
balance1 :: Tree α β -> α -> β -> Tree α β -> Tree α β
balance1 (Node _ (Node Red l kx vx r₁) ky vy r₂) kv vv t = Node Red (Node Black l kx vx r₁) ky vy (Node Black r₂ kv vv t)
balance1 (Node _ l₁ ky vy (Node Red l₂ kx vx r)) kv vv t = Node Red (Node Black l₁ ky vy l₂) kx vx (Node Black r kv vv t)
balance1 (Node _ l  ky vy r)                     kv vv t = Node Black (Node Red l ky vy r) kv vv t
balance1 _                                       _  _  _ = Leaf

balance2 :: Tree α β -> α -> β -> Tree α β -> Tree α β
balance2 t kv vv (Node _ (Node Red l kx₁ vx₁ r₁) ky vy r₂)  = Node Red (Node Black t kv vv l) kx₁ vx₁ (Node Black r₁ ky vy r₂)
balance2 t kv vv (Node _ l₁ ky vy (Node Red l₂ kx₂ vx₂ r₂)) = Node Red (Node Black t kv vv l₁) ky vy (Node Black l₂ kx₂ vx₂ r₂)
balance2 t kv vv (Node _ l ky vy r)                         = Node Black t kv vv (Node Red l ky vy r)
...
ins (Node Black a ky vy b) kx vx =
    if lt kx ky then
      (if is_red a then balance1 (ins a kx vx) ky vy b
       else Node Black (ins a kx vx) ky vy b)
    else if lt ky kx then
      (if is_red b then balance2 a ky vy (ins b kx vx)
