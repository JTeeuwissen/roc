...
size_t roc_str_len(struct RocStr* str) {
  char* bytes = (char*)str;
  struct RocStr dummy;
  char last_byte = bytes[sizeof(dummy) - 1];
...
extern void roc__mainForHost_1_exposed_generic(void* closure);
extern size_t roc__mainForHost_1_exposed_size();
extern void roc__mainForHost_0_caller(void* flags, void* closure, void* output);
...
int main() {
    ...
    void* output = roc_alloc(size, size);
...
int roc_fx_putLine(struct RocStr* str) {
